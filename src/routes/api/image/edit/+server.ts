...
export async function POST({ request }) {
  try {
    // Expect JSON body { prompt, file, mimeType }
    const { prompt, imageData: { base64Image, mimeType } } = await request.json();

    console.log("Prompt:", prompt);
    console.log("MimeType:", mimeType);

    if (!prompt || !base64Image || !mimeType) {
      return json({ error: "Missing 'prompt' or 'imageData' or 'mimeType'" }, { status: 400 });
    }
