...
<script lang="ts">
    ...
    import { templateContentMulti } from "$lib/apps/banana-studio/presets/component-template/templateContent";
    ...
    let preset = $derived(templateContentMulti.find(p => p.id === presetId));
    
    let Image1Files = $state<ImageFile[]>([]); // multiple files
    let Image2Files = $state<ImageFile[]>([]); // multiple files
    ...
    let isDraggingOver = $state({ image1: false, image2: false });
    ...
    async function handleFilesChange(e: Event, imageId: string) {
      ...
      const promises = Array.from(selectedFiles).map(async (selected) => {
        ...
        if (imageId === 'image1') {
          Image1Files = [...newFiles, ...Image1Files];
        } else {
          Image2Files = [...newFiles, ...Image2Files];
        }
    }
    
    function onDragOver(event: DragEvent, areaId: keyof typeof isDraggingOver) {
      event.preventDefault();
      isDraggingOver[areaId] = true;
    }
    
  function onDragLeave(event: DragEvent, areaId: keyof typeof isDraggingOver) {
    event.preventDefault();
    isDraggingOver[areaId] = false;
  }

  
    async function onDrop(event: DragEvent, areaId: keyof typeof isDraggingOver) {
      event.preventDefault();
    ...
    function removeFile(index: number, imageId: string) {
      if (imageId === 'image1') {
        Image1Files = Image1Files.filter((_, i) => i !== index);
      } else {
        Image2Files = Image2Files.filter((_, i) => i !== index);
      }
    }
    ...
    async function blendImages() {
      ...
      // Validate inputs
      if (!Image1Files.length || !Image2Files.length ) {
        errorMessage = "Please select at least one image for each image upload area and enter a prompt.";
        return;
      }
      ...
      try {
        const imagesData = Image1Files.concat(Image2Files)
        ...
        const res = await fetch("/api/image/blend-preset", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ presetId, prompt: cleanPrompt, imagesData })
        });
    ...
    <div class="px-6 md:px-8 lg:px-12 xl:px-16">
      <div class="py-8 md:py-12 lg:py-16 space-y-8 md:space-y-12">
        <header class="space-y-2 flex flex-col items-center">
          ...
          <p class="text-gray-600">{preset?.description || "Upload multiple images, describe the blend, and generate."}</p>
        </header>
    
        <!-- 2.1 Image Upload Area (Top) -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- Upload Area 1 -->
            {#if Image1Files && Image1Files.length}
            <div class="rounded-lg  p-6 md:p-8">
              {#each Image1Files as f, i}
                {#if f.dataURL}
                  <div class="relative bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all">
                    <img src={f.dataURL} alt={f.name} class="w-full  object-cover" />
                    <button class="absolute top-2 right-2 p-2 rounded-lg bg-white/90 text-gray-700 hover:bg-red-50 hover:text-red-600 border border-gray-200 transition-colors cursor-pointer" aria-label="Remove image" onclick={() => removeFile(i,'image1')}>
          ...
          {:else}
          <div
          class={`border-2 border-dashed rounded-xl transition-colors p-6 md:p-8 ${isDraggingOver.image1 ? 'bg-blue-50 border-blue-600' : 'bg-gray-50 border-gray-300'} hover:bg-blue-50 hover:border-blue-600`}
          ondragover={(e) => onDragOver(e, 'image1')}
          ondragleave={(e) => onDragLeave(e, 'image1')}
          ondrop={(e) => onDrop(e, 'image1')}
          aria-label="Image upload area 1"
          role="region"
        >
          <div class="flex flex-col items-center justify-center text-center space-y-3">
            ...
            <div>
              <label class="inline-flex items-center px-6 py-3 bg-white text-blue-900 border-2 border-blue-900 rounded-lg cursor-pointer hover:bg-blue-50 active:bg-blue-100 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 font-medium transition-colors">
                <input type="file" accept="image/*" multiple={false} onchange={ (e) => handleFilesChange(e,'image1')} class="hidden" />
                Browse Files
              </label>
            </div>
        ...
        {/if}
            ...
            <!-- Upload Area 2 -->
            {#if Image2Files && Image2Files.length}
            <div class="rounded-lg  p-6 md:p-8">
                {#each Image2Files as f, i}
                  {#if f.dataURL}
                    <div class="relative group bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all">
                      <img src={f.dataURL} alt={f.name} class="w-full object-cover" />
                      <button class="absolute top-2 right-2 p-2 rounded-lg bg-white/90 text-gray-700 hover:bg-red-50 hover:text-red-600 border border-gray-200 transition-colors cursor-pointer" aria-label="Remove image" onclick={() => removeFile(i,'image2')}>
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
                      </button>
                      <div class="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/40 to-transparent text-white text-xs px-2 py-1 truncate">{f.name}</div>
                    </div>
            ...
            {:else}
            <div
            class={`border-2 border-dashed rounded-xl transition-colors p-6 md:p-8 ${isDraggingOver.image2 ? 'bg-blue-50 border-blue-600' : 'bg-gray-50 border-gray-300'} hover:bg-blue-50 hover:border-blue-600`}
            ondragover={(e) => onDragOver(e, 'image2')}
            ondragleave={(e) => onDragLeave(e, 'image2')}
            ondrop={(e) => onDrop(e, 'image2')}
            aria-label="Image upload area 2"
            role="region"
          >
            <div class="flex flex-col items-center justify-center text-center space-y-3">
              <div class="text-blue-600">
              ...
              <div>
                <label class="inline-flex items-center px-6 py-3 bg-white text-blue-900 border-2 border-blue-900 rounded-lg cursor-pointer hover:bg-blue-50 active:bg-blue-100 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 font-medium transition-colors">
                  <input type="file" accept="image/*" multiple={false} onchange={ (e) => handleFilesChange(e,'image2')} class="hidden" />
                  Browse Files
                </label>
              </div>
              <p class="text-xs text-gray-500">PNG, JPG up to 10MB</p>
        ...
        {#if preset?.userInputRequired}
          <div class="p-6 md:p-8 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl">
            <textarea
              placeholder={preset?.userInputPlaceholder || "Describe how to blend the images..."}
              bind:value={prompt}
          ...
          <div class="mt-6 flex gap-4 justify-center">
            ...
            >
              ...
              {:else}
                {preset?.actionButtonText || "Generate Blend"}
              {/if}
            </button>
          </div>
        ...
        <section class="space-y-3">
          <h2 class="text-lg font-semibold text-gray-800">Generated Image History</h2>
          ...
          {:else}
            <p class="text-gray-500 text-sm">No generated images yet.</p>
          {/if}
        </section>
      </div>
    </div>
